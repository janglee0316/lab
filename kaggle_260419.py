# -*- coding: utf-8 -*-
"""
LEO Massive MIMO Channel Prediction — Kaggle/Jupyter Edition (v2)
=================================================================
...
v2 changes:
  - KF params 원본 복원:  kf_mag_clip=0.995, kf_Q=1e-2, kf_unit_circle=False
  - Samples/Epochs 증량:  train 60k, val 8k, epochs 300, patience 60
"""

# ============================================================
# 🔧 USER CONFIGURATION — 여기만 바꾸면 됩니다
# ============================================================
BASE_DIR            = "/kaggle/working/LEO_Project"
RUN_ABLATION        = True       # ablation 3개 모델 학습 여부
Q_IN                = 15         # 입력 슬롯 수
W_OUT               = 15         # 예측 슬롯 수
TRAIN_SAMPLES       = 60000      # 원본 90k → 60k
VAL_SAMPLES         = 8000       # 원본 10k → 8k
EPOCHS              = 300        # 원본 450 → 300
EARLY_STOP_PATIENCE = 60
BATCH_SIZE          = 128
NUM_WORKERS         = 2          # Kaggle CPU 활용
TAU_EFF_MS          = 5.0        # 효과적 CSI aging delay
DOP_MIN             = 50.0
DOP_MAX             = 100.0
TS_MS               = 1.0        # coherence interval (ms)
PILOT_SNR_DB        = 15.0
SCENARIO            = "baseline" # "baseline" | "tdnn_adv"
FORCE_REGEN_DATA    = False      # True 면 데이터셋 다시 생성
HORIZON_TRIALS      = 2000       # 평가용 trial 수
# ============================================================

import os, math, time, json, csv, random, shutil, sys, gc
...
def make_zip_of_folder(folder: str, zip_out: str):
    ...
    return shutil.make_archive(base_name, "zip", folder)
...
class SimCfg:
    ...
    # ── KalmanFilter (diag AR1) — 원본 기본값 복원 ──
    ...
    kf_unit_circle: bool = False   # False → mag_clip 방식 사용
    kf_mag_clip: float = 0.995     # ★ 원본 값 복원
    kf_init_P: float = 1.0
    kf_Q: float = 1e-2             # ★ 원본 값 복원
...
def dataset_path(base_dir, cfg, A):
    tag = (
        ...
        f"teff{safe_float_tag(cfg.tau_eff_ms)}_delay{cfg.delay_mode}_alt{cfg.fixed_alt_km}_"
        f"tr{cfg.train_samples}_va{cfg.val_samples}"
    )
    return os.path.join(base_dir, "datasets", f"dataset_{tag}.pt")

def generate_dataset_tensors(cfg, n_samples, A):
    print(f"⚡ Generating {n_samples} samples | q={cfg.q_in}, A={A}, L={cfg.w_out}", flush=True)
    ...
    t0 = time.time()
    log_every = max(500, n_samples // 20)
    for i in range(n_samples):
        ...
        if (i + 1) % log_every == 0:
            ...
            print(f"  {i+1}/{n_samples} | elapsed={elapsed:.1f}s ETA={eta:.1f}s", flush=True)
...
# ─────────────────────────────────────────────
# 4) KalmanFilter Baseline (Diagonal complex AR1)
# ─────────────────────────────────────────────
def complex_abs2(z):
...
class KalmanFilterDiagAR1:
    """
    Diagonal complex AR(1) KF baseline.
        x[n] = a * x[n-1] + w,   y[n] = x[n] + v

    Phase-only adaptive update with |a| clipped to kf_mag_clip (default 0.995).
    If kf_unit_circle=True is set, enforces |a|=1 strictly (bypasses mag_clip).
    """
    ...
    def _update_a(self, z):
        ...
        if bool(self.cfg.kf_unit_circle):
            # Strict |a|=1
    ...
    def update(self, y_vec):
        ...
        x_pri = self.a * self.x
        # |a|<=0.995 이므로 정확히는 |a|^2 * P 이지만, 원본과 동일하게 P + Q 사용
        P_pri = self.P + self.Q
...
# ─────────────────────────────────────────────
# 6) Training (with frequent prints + memory cleanup)
# ─────────────────────────────────────────────
def train_model(cfg, model, name, outdir, train_ds, val_ds, num_workers=2):
    print(f"\n🧠 Training {name} | q={cfg.q_in} | L={cfg.w_out} | ep={cfg.epochs} bs={cfg.batch_size}", flush=True)
    ...
    log_every = 5  # 자주 출력해서 idle timeout 방지
    ...
    for ep in range(cfg.epochs):
        ...
        if ep % log_every == 0 or ep == cfg.epochs - 1:
            ...
            print(f"[{name}] ep={ep:03d} train={tr:.6f} val={va:.6f} best={best_val:.6f} "
                  f"(bad={bad}/{cfg.early_stop_patience}) elapsed={elapsed:.0f}s", flush=True)
        if bad >= cfg.early_stop_patience:
            print(f"🟡 Early stop {name} @ep={ep}", flush=True)
            break

    model.load_state_dict(torch.load(best_path, map_location=cfg.device))
    print(f"✅ {name} best ckpt: {best_path} | size={file_exists_and_size(best_path)} | "
          f"total {time.time()-t_start:.0f}s", flush=True)

    # cleanup dataloaders
    del train_dl, val_dl, opt, scaler
    free_mem()
    return model
...
def eval_nmse_horizon(cfg, models, val_ds, A, trials=2000, kf=None):
    ...
    print(f"📐 NMSE eval over {len(idxs)} trials...", flush=True)
    t0 = time.time()
    log_every = max(200, len(idxs) // 10)
    ...
    for ii, idx in enumerate(idxs):
        ...
        if (ii + 1) % log_every == 0:
            print(f"  eval {ii+1}/{len(idxs)} | elapsed={time.time()-t0:.1f}s", flush=True)
...
# ─────────────────────────────────────────────
@torch.no_grad()
def measure_forward_latency_ms(model, cfg, q, D, iters=80):
...
@torch.no_grad()
def measure_forward_latency_cpu_ms(model, cfg, q, D, iters=60):
...
# ─────────────────────────────────────────────
def train_and_eval_one(base_dir, cfg, exp_tag, force_data=False, num_workers=2, run_ablation=True):
    ...
    try:
        ...
        print(f"⏱️  tau_eff={delay_ms:.2f} ms, Ts={cfg.coherence_ms:.2f} ms → A={A} slots")

        # Save config
        cfg_dump = asdict(cfg)
        cfg_dump["delay_calc"] = {"tau_eff_ms": float(delay_ms), "Ts_ms": float(cfg.coherence_ms), "A_slots": int(A)}
        with open(os.path.join(outdir, "cfg.json"), "w", encoding="utf-8") as f:
        ...
        print(f"🧩 TDNN blocks = {blocks} (q={cfg.q_in})")

        model_suite = build_model_suite(cfg, D=D, blocks=blocks, run_ablation=run_ablation)

        # ── Train each model with cleanup in between ──
        t0 = time.time()
        trained_models = {}
        for i, (name, model) in enumerate(model_suite.items(), 1):
            print(f"\n{'='*60}\n📦 Model {i}/{len(model_suite)}: {name}\n{'='*60}", flush=True)
            trained_models[name] = train_model(
                cfg, model, name.replace(" ", "_"), outdir,
                train_ds, val_ds, num_workers=num_workers).eval()
            free_mem()
        print(f"\n⏱️  Total training: {(time.time()-t0)/60.0:.2f} min")
        ...
        if cfg.make_aging_plots:
            ...
            plot_corr_curve(cfg, A, corr, corr_png, title="CSI Aging: Channel Correlation vs Lag")
        ...
        for name in ["TDNN-KalmanNet", "KalmanNet", "SCP"]:
            if name in trained_models:
                cpu_lat[name] = measure_forward_latency_cpu_ms(trained_models[name], cfg, cfg.q_in, D, iters=40)
        ...
        summary = {
            ...
            "nmse_at_w_first": {k: float(v[0]) for k, v in nmse_curve.items()},
            "nmse_at_w_last":  {k: float(v[-1]) for k, v in nmse_curve.items()},
            "params": params,
            ...
            "kf_config": {
                ...
                "unit_circle": cfg.kf_unit_circle, "mag_clip": cfg.kf_mag_clip,
            },
        }
        ...
        print("=" * 60)
        for k, v in nmse_curve.items():
            print(f"  {k:25s}  @w={A+1}: {v[0]:+.2f} dB   @w={A+cfg.w_out}: {v[-1]:+.2f} dB   mean: {np.mean(v):+.2f} dB")
        print()
        ...
        if latency_png:
            show_image(latency_png, "GPU Latency (ms)")
...
# ─────────────────────────────────────────────
# 12) Direct execution (no argparse)
# ─────────────────────────────────────────────
def run_experiment():
    ensure_dir(BASE_DIR)
    ...
    cfg = SimCfg(
        ...
        horizon_trials=HORIZON_TRIALS,
    )
    cfg = apply_scenario(cfg, SCENARIO)
    delay_ms, A = cfg.effective_delay_steps()

    tag = (
        f"{SCENARIO}_TEFF{safe_float_tag(delay_ms)}_q{cfg.q_in}_A{A}_L{cfg.w_out}_"
        f"dop{safe_float_tag(cfg.ut_doppler_hz_min)}to{safe_float_tag(cfg.ut_doppler_hz_max)}_"
    ...
    print(f"📌 q_in         = {cfg.q_in}")
    print(f"📌 w_out        = {cfg.w_out}")
    ...
    print(f"📌 tau_eff_ms   = {cfg.tau_eff_ms}  →  A={A} slots")
    print(f"📌 doppler      = {cfg.ut_doppler_hz_min}~{cfg.ut_doppler_hz_max} Hz")
    print(f"📌 Ts_ms        = {cfg.coherence_ms}")
    print(f"📌 scenario     = {SCENARIO}")
    print(f"📌 ablation     = {RUN_ABLATION}")
    print(f"📌 KF: mag_clip = {cfg.kf_mag_clip}, Q={cfg.kf_Q}, smooth={cfg.kf_a_smooth}, "
          f"unit_circle={cfg.kf_unit_circle}")
    print(f"📌 TDNN blocks  = {cfg.pick_tdnn_blocks()}")
    print("=" * 80)
    print(f"🚀 Tag: {tag}\n", flush=True)

    return train_and_eval_one(
        BASE_DIR, cfg, exp_tag=tag,
        force_data=FORCE_REGEN_DATA,
        num_workers=NUM_WORKERS,
        run_ablation=RUN_ABLATION,
    )


# Notebook 셀에서 import만 해도 되게 if-guard
if __name__ == "__main__":
    nmse_curve = run_experiment()
    print("\n🎉 All done!")

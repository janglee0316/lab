...
class SimCfg:
    ...
    # Stronger model-based baseline: block-diagonal linear KF
    kf_struct_block_size: int = 16
    kf_struct_ridge: float = 1e-3
    kf_struct_init_P: float = 1.0
    kf_struct_Q_jitter: float = 1e-4
    kf_struct_use_mmse_debias: bool = True
    # ---------------------------------------
...
class SimpleKalmanAR1_DiagComplex:
    ...
    def update(self, y_vec: torch.Tensor):
        ...
        x_pri = self.a * self.x
        P_pri = (torch.abs(self.a) ** 2) * self.P + self.Q
...
# -------------------------
# 4-B) Stronger Model-based Baseline: Structured Linear KF (Block-diagonal real state)
# -------------------------
def _safe_eye(n: int, scale: float = 1.0) -> torch.Tensor:
    return torch.eye(n, dtype=torch.float32) * float(scale)

def _iter_blocks(D: int, block_size: int):
    s = 0
    while s < D:
...
def fit_structured_linear_kf_from_trainX(Xtr: torch.Tensor, cfg: SimCfg):
    """
    Offline identification of a stronger linear KF baseline on the same normalized domain.
    State: x[n] in R^{2M}, Observation: y[n] = x[n] + v[n] after optional MMSE debias.
    Transition: x[n] = F x[n-1] + w[n], with block-diagonal F and Q.
    """
...
class StructuredLinearKF_BlockDiag:
    """
    Stronger model-based baseline:
      x[n] = F x[n-1] + w[n],  y[n] = x[n] + v[n]
    where F,Q,R are block-diagonal on the real-valued vectorized state.
    """
    def __init__(self, cfg: SimCfg, fit_pack: Dict):
        self.cfg = cfg
        self.D = cfg.feat_dim
        self.beta = float(fit_pack["beta"])
        self.F_blocks = [b.clone().float() for b in fit_pack["F_blocks"]]
        self.Q_blocks = [b.clone().float() for b in fit_pack["Q_blocks"]]
        self.R_blocks = [b.clone().float() for b in fit_pack["R_blocks"]]
        self.block_ranges = list(_iter_blocks(self.D, int(fit_pack["block_size"])))
        self.reset()

    def reset(self):
        self.x = None
        self.P_blocks = [
            _safe_eye(e - s, scale=float(self.cfg.kf_struct_init_P))
            for (s, e) in self.block_ranges
        ]

    def _prep_meas(self, y_vec: torch.Tensor) -> torch.Tensor:
        z = y_vec.float().cpu()
        if self.cfg.kf_struct_use_mmse_debias:
            z = z / max(1e-12, self.beta)
        return z
    ...
    def update(self, y_vec: torch.Tensor):
        ...
        x_new = []
        P_new = []
        for bi, (s, e) in enumerate(self.block_ranges):
            F = self.F_blocks[bi]
            Q = self.Q_blocks[bi]
            R = self.R_blocks[bi]
            P = self.P_blocks[bi]
            ...
            x_new.append(x_post)
            P_new.append(0.5 * (P_post + P_post.T))

        self.x = torch.cat(x_new, dim=0)
        self.P_blocks = P_new
...
class TDNNKalmanNet(nn.Module):
    def __init__(
        ...
        k: int = 3,
        drop: float = 0.1,
        feature_mode: str = "y_dy_ddy",
        prior_mode: str = "rotation",
    ...
    def _extract_features(self, x_in: torch.Tensor):
    ...
    def _rho_phi(self, head_out: torch.Tensor):
        rho = torch.sigmoid(head_out[:, 0:1]) * 1.25
        phi = math.pi * torch.tanh(head_out[:, 1:2])
        return rho, phi

    def _prior_step(self, x_post: torch.Tensor, rho: torch.Tensor, phi: torch.Tensor):
        if self.prior_mode == "identity":
            return x_post
        return rotate_realimag(x_post, rho, phi)

    def _gain(self, h_t: torch.Tensor):
        if self.gain_mode == "fixed":
            return torch.full((h_t.shape[0], self.D), self.fixed_gain, device=h_t.device, dtype=h_t.dtype)
    ...
    def forward(self, x_in: torch.Tensor, w_out: int):
        ...
        if self.rollout_mode == "gru":
            ...
            # Simple iterative prior-only rollout without recurrent refinement
            rp = self.fc_rp_r(h_r)
            rho, phi = self._rho_phi(rp)
            for _ in range(w_out):
                curr = self._prior_step(curr, rho, phi)
...
def build_model_suite(cfg: SimCfg, D: int, blocks: int, run_ablation: bool = False) -> Dict[str, nn.Module]:
    models: Dict[str, nn.Module] = {
        ...
        "KalmanNet-GRU": KalmanNetGRU(D, H=cfg.knet_hidden),
    ...
    if run_ablation:
        ...
        models["TDNN-fixedGain"] = TDNNKalmanNet(
            D, H=cfg.tdnn_hidden, blocks=blocks, k=cfg.tdnn_kernel, drop=cfg.tdnn_drop,
            feature_mode="y_dy_ddy", prior_mode="rotation", gain_mode="fixed", fixed_gain=0.5, rollout_mode="gru",
        )
...
# -------------------------
@torch.no_grad()
def eval_nmse_horizon(
    cfg: SimCfg,
    models: Dict[str, Optional[nn.Module]],
    val_ds: SliceDataset,
    A: int,
    trials: int = 2000,
    kf_diag: Optional[SimpleKalmanAR1_DiagComplex] = None,
    kf_struct: Optional[StructuredLinearKF_BlockDiag] = None,
):
    ...
    for idx in idxs:
        ...
        if kf_diag is not None:
            kf_diag.reset()
            for t in range(cfg.q_in):
                kf_diag.update(x_in[t].detach().cpu())
            preds["KF-diag-AR1"] = torch.stack(
                [kf_diag.predict_ahead(A + (i + 1)) for i in range(cfg.w_out)], dim=0
            ).to(cfg.device).float()

        if kf_struct is not None:
            kf_struct.reset()
            for t in range(cfg.q_in):
                kf_struct.update(x_in[t].detach().cpu())
            preds["KF-structured-linear"] = torch.stack(
                [kf_struct.predict_ahead(A + (i + 1)) for i in range(cfg.w_out)], dim=0
            ).to(cfg.device).float()
        ...
        for name, m in models.items():
            ...
            elif name in ("KF-diag-AR1", "KF-structured-linear"):
                continue
            else:
...
# -------------------------
# 8) Plotting + Eval Helpers
# -------------------------
@torch.no_grad()
def channel_corr_curve(cfg: SimCfg, max_lag: int, trials: int = 350) -> np.ndarray:
    # Measure temporal correlation of the channel
    ...
    for _ in range(trials):
        ...
        for _ in range(max_lag + 1):
            h_seq.append(chan.get_h_true().squeeze(0)) # (M,)
            chan.step()
        h_seq = torch.stack(h_seq) # (max_lag+1, M)
        
        # corr(tau) = |h(0)^H h(tau)| / (|h(0)| * |h(tau)|)
        h0 = h_seq[0]
        norm_h0 = torch.norm(h0)
        for tau in range(max_lag + 1):
            htau = h_seq[tau]
            c = torch.abs(torch.vdot(h0, htau)) / (norm_h0 * torch.norm(htau) + 1e-12)
            corr[tau] += c.item()
            
    return corr / trials

def plot_corr_curve(cfg: SimCfg, A: int, corr: np.ndarray, save_path: str, title: str):
    plt.figure(figsize=(8, 5))
    plt.plot(np.arange(len(corr)), corr, 'b-', linewidth=2)
    plt.axvline(x=A, color='r', linestyle='--', label=f'Delay A={A}')
    plt.grid(alpha=0.3)
    plt.xlabel("Time Lag (slots)")
...
def mrt_gain(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    # w = h_pred / ||h_pred||, gain = |h_true^H w|^2
    hc = devectorize_complex(target.float())
    pc = devectorize_complex(pred.float())
    norm_pc = torch.norm(pc, dim=-1).clamp_min(1e-12)
    w = pc / norm_pc.unsqueeze(-1)
    inner = torch.sum(torch.conj(hc) * w, dim=-1)
    return (torch.abs(inner) ** 2)

def spectral_efficiency_bpshz(gain: torch.Tensor, snr_db: float) -> torch.Tensor:
    snr_lin = 10 ** (snr_db / 10.0)
    return torch.log2(1.0 + snr_lin * gain)

@torch.no_grad()
def eval_rate_horizon(
    cfg: SimCfg,
    models: Dict[str, Optional[nn.Module]],
    val_ds: SliceDataset,
    A: int,
    ...
    for idx in idxs:
        ...
        for t in range(cfg.w_out):
            ...
            for name in models.keys():
                ...
                rate[name][t] += spectral_efficiency_bpshz(g, cfg.pilot_snr_db).item()
...
def approx_flops_models(cfg: SimCfg, tdnn_blocks: int) -> Dict[str, float]:
    ...
    f = 0
    f += approx_flops_conv1d(3 * D, Ht, 1, q)
    for _ in range(tdnn_blocks):
        f += approx_flops_conv1d(Ht, Ht, k, q)
        f += approx_flops_conv1d(Ht, Ht, k, q)
    ...
    f = 0
    f += q * approx_flops_linear(3 * D, Hg)
    f += q * approx_flops_gru_cell(Hg, Hg)
    f += q * (approx_flops_linear(Hg, 2) + approx_flops_linear(Hg, D))
...
def train_and_eval_one(
    ...
    try:
        ...
        print("\n🛠️ Fitting stronger model-based baseline: KF-structured-linear")
        kf_struct_fit = fit_structured_linear_kf_from_trainX(Xtr, cfg)
        kf_struct = StructuredLinearKF_BlockDiag(cfg, kf_struct_fit)
        kf_diag = SimpleKalmanAR1_DiagComplex(cfg=cfg)

        eval_models: Dict[str, Optional[nn.Module]] = {
            "TDNN-KalmanNet": trained_models["TDNN-KalmanNet"],
            "KalmanNet-GRU": trained_models["KalmanNet-GRU"],
            "KF-diag-AR1": None,
            "KF-structured-linear": None,
            "SCP": trained_models["SCP"],
            "Outdated": None,
        }
        if run_ablation:
            for k in ("TDNN-noDelta", "TDNN-noPrior", "TDNN-fixedGain", "TDNN-noRollGRU"):
                eval_models[k] = trained_models[k]

        nmse_curve = eval_nmse_horizon(
            cfg, eval_models, val_ds, A=A, trials=cfg.horizon_trials,
            kf_diag=kf_diag, kf_struct=kf_struct
        )
        ...
        rate_curve = eval_rate_horizon(
            cfg, eval_models, val_ds, A=A, trials=cfg.horizon_trials,
            kf_diag=kf_diag, kf_struct=kf_struct
        )
        rate_raw_csv = os.path.join(outdir, "rate_vs_predslots_raw.csv")
        save_csv_nmse_raw(rate_raw_csv, rate_curve)
        rate_png = os.path.join(outdir, "rate_vs_w_axis_AplusT.png")
        plot_rate_curve_w_axis(
            cfg, A, rate_curve, rate_png,
        ...
        params["KF-diag-AR1"] = 0
        params["KF-structured-linear"] = 0
        params["Outdated"] = 0
        ...
        size_mb = {}
        for name, model in trained_models.items():
            size_mb[f"{name}(fp32)"] = bytes_to_mb(state_dict_size_bytes(model, fp16=False))
            size_mb[f"{name}(fp16)"] = bytes_to_mb(state_dict_size_bytes(model, fp16=True))
        ...
        if cfg.measure_latency and cfg.device.startswith("cuda"):
            for name, model in trained_models.items():
                latency[name] = measure_forward_latency_ms(model, cfg, cfg.q_in, D, iters=80)
            ...
            json.dump(
                {
                    ...
                    "kf_struct": {
                        "block_size": cfg.kf_struct_block_size,
                        "ridge": cfg.kf_struct_ridge,
                        "init_P": cfg.kf_struct_init_P,
                        "Q_jitter": cfg.kf_struct_Q_jitter,
                        "use_mmse_debias": cfg.kf_struct_use_mmse_debias,
                    },
                    "params": params,
...
def apply_scenario(cfg: SimCfg, scenario: str) -> SimCfg:
    ...
    if scenario == "tdnn_adv":
        return replace(
            ...
            lr=min(cfg.lr, 8e-4),
            # Harder mismatch -> simple KF usually needs a slightly larger Q
            kf_a_smooth=0.10,
            kf_R_scale=1.0,
            kf_Q=max(cfg.kf_Q, 1e-2)
        )
    ...
    if scenario == "tdnn_adv":
        return replace(
            ...
            # --- [추가/수정된 부분] ---
            # 1. 파라미터 역전을 위한 은닉층 차원 조절
            knet_hidden=384,        # KalmanNet은 파라미터를 늘림
            tdnn_hidden=128,        # TDNN은 파라미터를 대폭 줄임
            
            # 2. Kalman Filter 1dB 성능 향상 튜닝
            kf_a_smooth=0.10,       # 더 빠른 적응력
            kf_R_scale=1.0,         # 측정값 신뢰도 복구
            kf_Q=max(cfg.kf_Q, 1e-2) # 프로세스 노이즈 상향
        )

...
class SimCfg:
    ...
    kf_mag_clip: float = 1.0       # ★ FIX: was 0.995 → now 1.0 (unit circle)

    kf_init_P: float = 1.0
    kf_Q: float = 5e-3             # ★ FIX: was 1e-2 → 5e-3 (|a|=1이므로)
...
class KalmanFilterDiagAR1:
    ...
    def __init__(self, cfg: SimCfg):
        ...
        # ★ FIX 1: Always init from Doppler center (phase rotation on unit circle)
    ...
    def _update_a(self, z):
        ...
        # ★ FIX 2: |a| ≡ 1.0 always (unit circle constraint)
...
def build_model_suite(cfg, D, blocks, run_ablation=False):
    models = {
...
def main():
    ...
    parser.add_argument("--q_list", type=str, default="4,15")

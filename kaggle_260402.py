# -*- coding: utf-8 -*-
"""
LEO Massive MIMO Channel Prediction — Kaggle/Notebook-Friendly Version
...
Notebook usage:
  - In Kaggle / Jupyter, just run this file or paste into a cell.
  - The NOTEBOOK_DEFAULTS block below is used automatically.
...
# ============================================================
# Notebook defaults (used automatically in Kaggle / Jupyter)
# ============================================================
NOTEBOOK_DEFAULTS = {
    ...
    "dop_max": 100.0,
    "Ts_ms": 1.0,
    "scenario": "baseline",   # choices: baseline, tdnn_adv
    "no_latency": False,
    "ablation": True,
}
...
class SimCfg:
    ...
    # ModernTCN-inspired baseline
    modern_hidden: int = 192
    modern_blocks_small: int = 2
    modern_blocks_large: int = 4
    modern_kernel: int = 9
    modern_expand: int = 2
    modern_drop: float = 0.10
    ...
    def pick_modern_blocks(self) -> int:
        return self.modern_blocks_small if self.q_in <= 7 else self.modern_blocks_large
...
class ModernTCNBlock(nn.Module):
    def __init__(self, ch, kernel=9, expand=2, drop=0.1):
        ...
        pad = kernel // 2
        hidden = ch * expand
        self.dw = nn.Conv1d(ch, ch, kernel_size=kernel, padding=pad, groups=ch, padding_mode="replicate")
        self.norm1 = nn.BatchNorm1d(ch)
        self.pw1 = nn.Conv1d(ch, hidden, kernel_size=1)
        self.act = nn.GELU()
        self.drop1 = nn.Dropout(drop)
        self.pw2 = nn.Conv1d(hidden, ch, kernel_size=1)
    ...
    def forward(self, x):
        ...
        y = self.drop2(y)
        out = (x + y).transpose(1, 2)
        return self.norm2(out).transpose(1, 2)
...
class ModernTCNEncoder(nn.Module):
    def __init__(self, in_ch, H, blocks, kernel=9, expand=2, drop=0.1):
        ...
        self.blocks = nn.ModuleList([
            ModernTCNBlock(H, kernel=kernel, expand=expand, drop=drop) for _ in range(blocks)
        ])
        self.out_ln = nn.LayerNorm(H)
...
class ModernTCNKalmanNet(nn.Module):
    def __init__(self, D, H=192, blocks=2, kernel=9, expand=2, drop=0.1,
        ...
        in_mult = 3 if feature_mode == "y_dy_ddy" else 1
        self.enc = ModernTCNEncoder(in_ch=in_mult * D, H=H, blocks=blocks, kernel=kernel, expand=expand, drop=drop)
        self.fc_gain = nn.Linear(H, D)
        self.fc_rp = nn.Linear(H, 2)
        self.roll_in = nn.Linear(H + D, H)
        self.roll_gru = nn.GRUCell(H, H)
...
def build_model_suite(cfg, D, tdnn_blocks, modern_blocks, run_ablation=False):
    models = {
        "TDNN-KalmanNet": TDNNKalmanNet(
            D, H=cfg.tdnn_hidden, blocks=tdnn_blocks, k=cfg.tdnn_kernel, drop=cfg.tdnn_drop,
            feature_mode="y_dy_ddy", prior_mode="rotation", gain_mode="learned", rollout_mode="gru"
        ),
        "ModernTCN-KalmanNet": ModernTCNKalmanNet(
            D, H=cfg.modern_hidden, blocks=modern_blocks, kernel=cfg.modern_kernel,
            expand=cfg.modern_expand, drop=cfg.modern_drop,
            feature_mode="y_dy_ddy", prior_mode="rotation", gain_mode="learned", rollout_mode="gru"
        ),
...
def plot_nmse_curve_w_axis(cfg, A, nmse_curve, save_path, title):
    ...
    plt.title(title + f"\ntau_eff={delay_ms:.2f}ms | Ts={cfg.coherence_ms:.2f}ms | dop={cfg.ut_doppler_hz_min}-{cfg.ut_doppler_hz_max}Hz | A={A}")
    plt.legend()
    plt.tight_layout()
    plt.savefig(save_path, dpi=200)
    plt.close()
...
def approx_flops_models(cfg, tdnn_blocks, modern_blocks):
    D = cfg.feat_dim
    Ht = cfg.tdnn_hidden
    Hm = cfg.modern_hidden
    Hg = cfg.knet_hidden
    Hs = cfg.scp_hidden
    q = cfg.q_in
    L = cfg.w_out
    k = cfg.tdnn_kernel
    km = cfg.modern_kernel
    flops = {}
    ...
    f = 0
    f += 2 * q * (3 * D) * Hm
    for _ in range(modern_blocks):
        f += 2 * q * Hm * km
        f += 2 * q * Hm * (Hm * cfg.modern_expand)
        f += 2 * q * (Hm * cfg.modern_expand) * Hm
    f += q * (2 * Hm * D + 2 * Hm * 2)
    f += L * (2 * (Hm + D) * Hm + 2 * 3 * (Hm * Hm + Hm * Hm) + 2 * Hm * 2)
...
def train_and_eval_one(base_dir, cfg, exp_tag, force_data=False, num_workers=0, run_ablation=False):
    ...
    try:
        ...
        D = cfg.feat_dim
        tdnn_blocks = cfg.pick_tdnn_blocks()
        modern_blocks = cfg.pick_modern_blocks()
        print(f"\n🧩 TDNN blocks = {tdnn_blocks} (q={cfg.q_in})")
        print(f"🧩 ModernTCN blocks = {modern_blocks} (q={cfg.q_in})")

        model_suite = build_model_suite(cfg, D=D, tdnn_blocks=tdnn_blocks, modern_blocks=modern_blocks, run_ablation=run_ablation)
        ...
        eval_models = {
            ...
            "ModernTCN-KalmanNet": trained_models["ModernTCN-KalmanNet"],
        ...
        flops_g = approx_flops_models(cfg, tdnn_blocks=tdnn_blocks, modern_blocks=modern_blocks)
        ...
        for name, model in trained_models.items():
            if not name.startswith("TDNN-no"):
        ...
        size_mb = {}
        for name in ["TDNN-KalmanNet", "ModernTCN-KalmanNet", "KalmanNet", "SCP"]:
        ...
        summary = {
            ...
            "kf_config": {
                "debias_mmse": cfg.kf_debias_mmse,
                "a_mode": cfg.kf_a_mode,
                "a_smooth": cfg.kf_a_smooth,
                "Q": cfg.kf_Q,
                "R_scale": cfg.kf_R_scale,
...
def build_args():
    ...
    parser.add_argument("--base_dir", type=str, default=NOTEBOOK_DEFAULTS["base_dir"])
    parser.add_argument("--force_data", action="store_true")
    parser.add_argument("--q_list", type=str, default=NOTEBOOK_DEFAULTS["q_list"])
    parser.add_argument("--num_workers", type=int, default=NOTEBOOK_DEFAULTS["num_workers"])
    parser.add_argument("--tau_eff_ms", type=float, default=NOTEBOOK_DEFAULTS["tau_eff_ms"])
    parser.add_argument("--alt_km", type=int, default=NOTEBOOK_DEFAULTS["alt_km"])
    parser.add_argument("--w_out", type=int, default=NOTEBOOK_DEFAULTS["w_out"])
    parser.add_argument("--dop_min", type=float, default=NOTEBOOK_DEFAULTS["dop_min"])
    parser.add_argument("--dop_max", type=float, default=NOTEBOOK_DEFAULTS["dop_max"])
    parser.add_argument("--Ts_ms", type=float, default=NOTEBOOK_DEFAULTS["Ts_ms"])
    parser.add_argument("--scenario", type=str, default=NOTEBOOK_DEFAULTS["scenario"], choices=["baseline", "tdnn_adv"])
...
def main():
    ...
    for q in q_values:
        ...
        print(f"   ModernTCN blocks = {cfg.pick_modern_blocks()} (q={cfg.q_in})")

...
This version (per your request):
1) Fix altitude at 1500 km (used for delay axis).
2) Compute delay steps for alt=1500 km and print/save it.
3) Set output horizon w_out = 15.
4) Save all console prints to a separate log file (run.log) per experiment folder.
5) Keep the dataset-based training (MMSE-estimated inputs, TRUE CSI targets).
6) Keep Outdated baseline as LAST OBSERVED (MMSE-estimated) CSI, fixed across horizon.

Notes:
- The channel generator itself does not explicitly use altitude; altitude here is used in the delay-axis model
  (same as your current code structure).
- Delay model (as in your code): delay_ms = alt_km/300; if roundtrip, multiply by 2.
"""
...
class SimCfg:
    ...
    # Sequence
    q_in: int = 4
    w_out: int = 15  # <<< output horizon fixed to 15
    ...
    # Delay settings
    delay_mode: str = "roundtrip"  # "oneway" or "roundtrip"

    # FIXED altitude for delay axis
    ...
    def delay_steps_from_alt_km(self, alt_km: int) -> Tuple[float, int]:
        # delay_ms = alt_km/300 (your simplified model)
...
def eval_nmse_horizon(cfg: SimCfg, models: Dict[str, Optional[nn.Module]], val_ds: SliceDataset, trials: int = 2000):
    ...
    for idx in idxs:
        ...
        # Outdated baseline: last OBSERVED (MMSE-estimated) CSI at time q_in, keep it fixed
...
def save_csv_nmse_delay_axis(save_path: str, w_axis: np.ndarray, nmse_curve: Dict[str, np.ndarray]):
    keys = list(nmse_curve.keys())
    with open(save_path, "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["w_delay_axis"] + keys)
        for i in range(len(w_axis)):
            w.writerow([int(w_axis[i])] + [float(nmse_curve[k][i]) for k in keys])

def plot_nmse_curve_delay_axis_full(
    cfg: SimCfg,
    ...
    Plot NMSE using delay-aware x-axis:
      w = delay_steps(fixed_alt_km) + predicted_slot (1..w_out)
    Here we plot FULL horizon (15 points), no cropping.
    """
    delay_ms, delay_steps = cfg.delay_steps_from_alt_km(cfg.fixed_alt_km)
    w_axis = delay_steps + np.arange(1, cfg.w_out + 1)
    ...
    plt.ylabel("NMSE (dB)")
    plt.title(title + f" | alt={cfg.fixed_alt_km}km, delay_mode={cfg.delay_mode}, delay_ms={delay_ms:.2f}, steps={delay_steps}")
    plt.legend()
    plt.tight_layout()
...
def train_and_eval_one(base_dir: str, cfg: SimCfg, exp_tag: str, force_data: bool = False, num_workers: int = 0):
    ...
    try:
        ...
        # Delay calculation for altitude=1500km (fixed)
        delay_ms, delay_steps = cfg.delay_steps_from_alt_km(cfg.fixed_alt_km)
        print(f"\n⏱️ Delay (fixed alt): alt={cfg.fixed_alt_km} km | delay_mode={cfg.delay_mode}")
        print(f"   delay_ms = {delay_ms:.2f} ms  | coherence_ms = {cfg.coherence_ms:.2f} ms  -> delay_steps = {delay_steps}")

        # Save cfg (including delay result)
        cfg_dump = asdict(cfg)
        cfg_dump["delay_calc"] = {
            "alt_km": cfg.fixed_alt_km,
            "delay_mode": cfg.delay_mode,
            "delay_ms": float(delay_ms),
            "coherence_ms": float(cfg.coherence_ms),
        ...
        # Delay-aware axis (w = delay_steps + t)
        w_axis = delay_steps + np.arange(1, cfg.w_out + 1)
        nmse_w_csv = os.path.join(outdir, "nmse_vs_w_delay_axis.csv")
        save_csv_nmse_delay_axis(nmse_w_csv, w_axis, nmse_curve)

        # Plot (FULL 15 points, delay-aware x-axis)
        nmse_png = os.path.join(outdir, "nmse_vs_w_delay_axis_full15.png")
        plot_nmse_curve_delay_axis_full(
            cfg, nmse_curve,
            save_path=nmse_png,
            ...
            json.dump(
                {
                    "delay_ms": float(delay_ms),
                    "delay_steps": int(delay_steps),
                    "w_axis": [int(x) for x in w_axis.tolist()],
                    "params": params,
                },
                f,
                indent=2,
        ...
        for p in [nmse_png, nmse_raw_csv, nmse_w_csv, params_png, summary_path, log_path]:
            print("  -", p, "|", file_exists_and_size(p))

        show_image(nmse_png, "NMSE vs w (delay-aware) [FULL 15]")
...
def main():
    ...
    parser.add_argument("--w_out", type=int, default=15, help="prediction horizon (default 15)")
    ...
    print(f"📌 w_out        = {args.w_out} (FULL horizon points)")
    ...
    for q in q_values:
        ...
        tag = (
            f"ALT{cfg.fixed_alt_km}_q{cfg.q_in}_L{cfg.w_out}_dop{int(cfg.max_ut_doppler_hz)}_"
            f"coh{safe_float_tag(cfg.coherence_ms)}_snr{int(cfg.pilot_snr_db)}_"
            f"delay{cfg.delay_mode}_dSteps{delay_steps}_"
            f"{timestamp_str()}"
        )

...
1) Outdated baseline FIX:
   - Before: used perfect TRUE CSI at first future slot -> unrealistic (-120 dB at t=1)
   - Now: uses LAST OBSERVED (MMSE-estimated) CSI and keeps it fixed (true "outdated CSI")

2) NMSE vs Pred Slots plot:
   - Use delay-aware x-axis "w-th coherence interval" = delay_steps + predicted_slot
   - Plot only w=6..25 (paper-like view)

3) Run two experiments in one script: q=4 and q=20

4) Training hyperparams bumped to paper-scale defaults:
   train=90k, val=10k, epochs=500, batch=100, lr=1e-3

5) Inference-time plot REMOVED

6) Focus altitude=1500 km delay axis for NMSE plot (still can keep delay-vs-alt figure if you want)

Notes:
- This script still generates dataset (MMSE-estimated inputs, TRUE CSI targets).
- Default coherence_ms=1.0, dop=100 Hz, snr=15 dB (paper-aligned style).
"""
...
class SimCfg:
    ...
    # Dataset size (paper-scale defaults)
    train_samples: int = 80000
    val_samples: int = 9000
    ...
    # TDNN-KalmanNet
    # Auto-select layers based on q:
    #   q<=7  -> 2 layers (dil=1,2), RF≈7
    #   q>7   -> 4 layers (dil=1,2,4,8), RF≈31
    tdnn_hidden: int = 256
    ...
    # Delay/altitude settings
    altitudes_km: Tuple[int, ...] = (500, 1000, 1500)
    delay_mode: str = "oneway"  # "oneway" or "roundtrip"

    # "Paper-like" NMSE plot window on delay-aware x-axis w
    # (w = delay_steps + predicted_slot_index)
    plot_w_min: int = 6
    plot_w_max: int = 25
    plot_ref_alt_km: int = 1500  # use this altitude to define delay_steps for the NMSE-vs-slots x-axis
    ...
    def delay_steps_from_alt_km(self, alt_km: int) -> int:
        # Simple delay model used in your code: delay_ms = alt_km/300
...
def generate_dataset_tensors(cfg: SimCfg, n_samples: int):
    ...
    for i in range(n_samples):
        ...
        x_raw = seq_obs[:cfg.q_in]         # OBS (MMSE estimate)
...
def eval_nmse_horizon(cfg: SimCfg, models: Dict[str, Optional[nn.Module]], val_ds: SliceDataset, trials: int = 2000):
    """
    Returns nmse curves indexed by predicted_slot t=1..w_out (not delay-shifted).
    Delay-shifted x-axis plotting will be handled in plotting stage.
    """
    ...
    for idx in idxs:
        ...
        # Outdated baseline (FIXED):
        # use last OBSERVED (MMSE-estimated) CSI at time q_in, keep it fixed
...
# 9) Plotting
# -------------------------
def plot_nmse_curve_delay_axis(
    cfg: SimCfg,
    ...
    """
    Plot NMSE using delay-aware x-axis:
      w = delay_steps(ref_alt_km) + predicted_slot (1..w_out)
    Then show only w in [w_min, w_max].
    """
    delay_steps = cfg.delay_steps_from_alt_km(ref_alt_km)
    w_axis = delay_steps + np.arange(1, cfg.w_out + 1)  # e.g., 1500km(oneway), coh=1ms -> delay≈5 -> w=6..35

    mask = (w_axis >= w_min) & (w_axis <= w_max)
    w_plot = w_axis[mask]

    plt.figure(figsize=(10, 6))
    for k, v in nmse_curve.items():
        plt.plot(w_plot, v[mask], marker='o', linewidth=2, label=k)

    plt.grid(alpha=0.3)
    plt.xlabel("w-th coherence interval (delay-aware)")
    plt.ylabel("NMSE (dB)")
    plt.title(title + f" | ref_alt={ref_alt_km}km, delay_steps={delay_steps}, show w={w_min}..{w_max}")
    plt.legend()
    plt.tight_layout()
...
def train_and_eval_one(base_dir: str, cfg: SimCfg, exp_tag: str, force_data: bool = False, num_workers: int = 0):
    ...
    # Save raw curve (t=1..L)
    nmse_csv = os.path.join(outdir, "nmse_vs_predslots_raw.csv")
    save_csv_nmse(nmse_csv, nmse_curve)

    # Delay-aware x-axis plot (paper-like window)
    nmse_png = os.path.join(outdir, "nmse_vs_w_interval_delay_axis_w6to25.png")
    plot_nmse_curve_delay_axis(
        cfg, nmse_curve,
        save_path=nmse_png,
        title=f"NMSE vs w (delay-aware) | q={cfg.q_in}, L={cfg.w_out} | dop={cfg.max_ut_doppler_hz}, coh={cfg.coherence_ms}, snr={cfg.pilot_snr_db}",
        ref_alt_km=cfg.plot_ref_alt_km,
        w_min=cfg.plot_w_min,
        w_max=cfg.plot_w_max
    )

    # Delay@Altitude plot (kept)
    delay_png = os.path.join(outdir, "nmse_at_delay_vs_altitude.png")
    plot_delay_vs_alt(
        cfg, nmse_curve,
        save_path=delay_png,
        title=f"NMSE@Delay vs Altitude | q={cfg.q_in}, L={cfg.w_out} | coh={cfg.coherence_ms}, delay_mode={cfg.delay_mode}"
    )
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump({"params": params}, f, indent=2, ensure_ascii=False)

    print("\n✅ Saved results to:", outdir)
    for p in [nmse_png, nmse_csv, delay_png, params_png, summary_path]:
        print("  -", p, "|", file_exists_and_size(p))

    show_image(nmse_png, "NMSE vs w (delay-aware) [w=6..25]")
...
def main():
    ...
    parser.add_argument("--q_list", type=str, default="4,20", help="comma-separated q values, e.g., 4,20")
    parser.add_argument("--num_workers", type=int, default=0, help="DataLoader num_workers")
    parser.add_argument("--delay_mode", type=str, default="oneway", choices=["oneway", "roundtrip"])
    parser.add_argument("--plot_ref_alt_km", type=int, default=1500, help="reference altitude for delay-aware NMSE x-axis")
    parser.add_argument("--plot_w_min", type=int, default=6)
    parser.add_argument("--plot_w_max", type=int, default=25)
    ...
    print(f"📌 delay_mode      = {args.delay_mode}")
    print(f"📌 plot_ref_alt_km  = {args.plot_ref_alt_km}  (delay-aware NMSE axis)")
    print(f"📌 plot window w    = {args.plot_w_min}..{args.plot_w_max}")
    ...
    for q in q_values:
        cfg = replace(
            ...
            plot_ref_alt_km=int(args.plot_ref_alt_km),
            plot_w_min=int(args.plot_w_min),
            plot_w_max=int(args.plot_w_max),
        )

        # tag per q
        tag = (
            f"EXP_q{cfg.q_in}_L{cfg.w_out}_dop{int(cfg.max_ut_doppler_hz)}_"
            f"coh{safe_float_tag(cfg.coherence_ms)}_snr{int(cfg.pilot_snr_db)}_"
            f"delay{cfg.delay_mode}_plotAlt{cfg.plot_ref_alt_km}_"
            f"{timestamp_str()}"
        )
        print("\n" + "=" * 80)
        print(f"🚀 Running: {tag}")

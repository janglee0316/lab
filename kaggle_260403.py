# -*- coding: utf-8 -*-
"""
LEO Massive MIMO Channel Prediction — TDNN-KalmanNet
Platform: Kaggle notebook (also works on Colab/local)

Comparison (5 methods):
  TDNN-KalmanNet | KalmanNet (GRU) | SCP (LSTM) |
  KalmanFilter | Outdated CSI

Ablation (3 variants):
  TDNN-noPrior | TDNN-noDelta | TDNN-noRollGRU

Key design choices:
  - TDNN encoder: dilated TCN with residual blocks
  - KalmanFilter: unit-circle AR(1), Doppler-init, phase-only adaptive
  - LR: linear warmup (10ep) + cosine decay
  - Kaggle defaults: q=4, no correlation plot, ablation enabled
"""
...
def build_model_suite(cfg, D, tdnn_blocks, run_ablation=False):
    models = {
...
def train_and_eval_one(base_dir, cfg, exp_tag, force_data=False, num_workers=0, run_ablation=False):
    ...
    try:
        ...
        print(f"🧩 TDNN blocks={tb} (q={cfg.q_in})")

        model_suite = build_model_suite(cfg, D, tb, run_ablation)
        ...
        nmse_main_curve = eval_nmse_horizon(cfg, eval_models_main, val_ds, A, cfg.horizon_trials, kf)
        eval_models = dict(eval_models_main)
        if run_ablation:
        ...
        flops = approx_flops_models(cfg, tb)
        ...
        cpu_lat = {}
        for n in ["TDNN-KalmanNet", "KalmanNet", "SCP"]:
            if n in trained:
        ...
        main_models_keys = ["TDNN-KalmanNet", "KalmanNet", "SCP", "KalmanFilter", "Outdated"]
        ...
        summary = {
            ...
            "mean_nmse_main": {k: float(np.mean(v)) for k, v in nmse_main_curve.items()},
            "nmse_w_first_main": {k: float(v[0]) for k, v in nmse_main_curve.items()},
            "nmse_w_last_main": {k: float(v[-1]) for k, v in nmse_main_curve.items()},
            "mean_nmse_all": {k: float(np.mean(v)) for k, v in nmse_curve.items()},
            "nmse_w_first_all": {k: float(v[0]) for k, v in nmse_curve.items()},
            "nmse_w_last_all": {k: float(v[-1]) for k, v in nmse_curve.items()},
